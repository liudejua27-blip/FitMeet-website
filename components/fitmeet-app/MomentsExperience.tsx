...
function MomentAvatar({ post, onUser }: { post: FeedPost; onUser: (id: number) => void }) {
  return (
    <button
      type="button"
      className={styles.momentAvatar}
      style={{ '--moment-color': post.color || '#4f7cff' } as React.CSSProperties}
      aria-label={`查看${post.username}的资料`}
      onClick={() => onUser(Number(post.userId))}
    >
...
function DiscoveryHall({
  ...
  task: boolean;
  intents: FitMeetPublicIntent[];
  applications: FitMeetIntentApplication[];
  onAction: (
  ...
  const [draftIntentId, setDraftIntentId] = useState<string | null>(null);
  const [applicationMessage, setApplicationMessage] = useState('');
  const [submittingIntentId, setSubmittingIntentId] = useState<string | null>(null);
  if (!intents.length)
    return (
    ...
    <section className={styles.discoveryHall}>
      ...
      {intents.map((intent) => {
        const application = applications.find(
          (item) => (task ? item.taskIntentId : item.publicIntentId) === intent.id,
        );
            ...
            : status === 'pending'
              ...
              : status === 'accepted'
                ...
                : status === 'rejected'
                  ? '对方暂未接受'
                  : '重新申请';
        const composing = draftIntentId === intent.id;
        return (
          <article key={intent.id}>
            <span>{task ? '任务大厅' : '社交大厅'}</span>
            <h2>{intent.title || (task ? '服务需求' : '社交需求')}</h2>
            ...
            {composing ? (
              <form
                className={styles.intentApplicationForm}
                onSubmit={(event) => {
                  event.preventDefault();
                  if (!applicationMessage.trim() || submittingIntentId) return;
                  setSubmittingIntentId(intent.id);
                  void onAction(intent, 'pending', applicationMessage.trim()).finally(() => {
              ...
              >
                ...
                <div>
                  ...
                  <button
                    type="submit"
                    disabled={!applicationMessage.trim() || submittingIntentId === intent.id}
                  >
                    {submittingIntentId === intent.id ? '提交中…' : '确认提交'}
                  </button>
                </div>
              </form>
            ) : (
              <button
                ...
                disabled={
                  ['accepted', 'rejected'].includes(status) || submittingIntentId === intent.id
                }
                onClick={() => {
                  if (status === 'pending') {
                    setSubmittingIntentId(intent.id);
                    void onAction(intent, 'cancelled').finally(() => setSubmittingIntentId(null));
                  } else {
                    setDraftIntentId(intent.id);
                    setApplicationMessage(
                      task ? '我可以先确认任务范围和交付边界。' : '想先了解活动节奏和参与边界。',
                    );
                  }
                }}
              >
                {submittingIntentId === intent.id
                  ? '提交中…'
                  : status === 'pending'
                    ? '取消申请'
                    : action}{' '}
...
export function MomentsExperience({
  ...
  const [commentPages, setCommentPages] = useState<
    Record<number, { page: number; lastPage: number }>
  >({});
  ...
  const visiblePosts = useMemo(
    () => posts.filter((post) => !hiddenIds.includes(post.id)),
    [hiddenIds, posts],
  );
  ...
  const sharePost = async (post: FeedPost) => {
    const url = new URL(`/agent/try/discover/posts/${post.id}`, window.location.origin);
    const payload = {
      title: `${post.username} 的 FitMeet 动态`,
      text: post.text,
      url: url.toString(),
    };
    try {
    ...
    try {
        ...
        [postId]:
          page === 1
            ...
            : [
                ...(current[postId] || []),
                ...result.data.filter(
                  (item) => !(current[postId] || []).some((existing) => existing.id === item.id),
                ),
              ],
      }));
      setCommentPages((current) => ({
        ...
        [postId]: {
          ...
          lastPage: result.metadata?.lastPage ?? page,
        },
      }));
    } catch (reason) {
      onNotice(reason instanceof Error ? reason.message : '评论暂时无法加载。');
    } finally {
  ...
  const submitComment = async (postId: number) => {
    ...
    try {
      ...
      onNotice(reason instanceof Error ? reason.message : '评论未能发布。');
    }
  };
  ...
  const deleteComment = async (comment: FeedComment) => {
    try {
      ...
      onNotice(reason instanceof Error ? reason.message : '评论未能删除。');
    }
  };
  ...
  const reportComment = async (comment: FeedComment) => {
    try {
      ...
      onNotice(reason instanceof Error ? reason.message : '评论举报未能提交。');
    }
  };
  ...
  const changeFeedScope = async (scope: 'all' | 'friends') => {
    ...
    try {
      ...
      onNotice(reason instanceof Error ? reason.message : '动态列表暂时无法切换。');
    }
  };
  ...
  const loadMore = async () => {
    ...
    try {
      const page: FeedPage =
        feedScope === 'friends'
          ? await api.getFriendFeed(currentPage + 1, 10)
          : await api.getFeed(currentPage + 1, 10);
  ...
  return (
    <div className={styles.standardScreen}>
      ...
      <div className={styles.segmented}>
        {(['moments', 'social', 'tasks'] as const).map((item) => (
          <button
            type="button"
            key={item}
            className={channel === item ? styles.segmentActive : ''}
            onClick={() => onChannel(item)}
          >
            {item === 'moments' ? '朋友圈' : item === 'social' ? '社交大厅' : '任务大厅'}
          </button>
        ))}
      </div>
      {channel !== 'moments' ? (
        <DiscoveryHall
          task={channel === 'tasks'}
          intents={channel === 'tasks' ? taskIntents : socialIntents}
          applications={channel === 'tasks' ? taskApplications : socialApplications}
          onAction={(intent, status, message) =>
            onApplication(channel === 'tasks' ? 'task' : 'social', intent, status, message)
          }
        />
      ) : (
        <>
          <div className={styles.feedScope} aria-label="动态范围">
            <button
              type="button"
              className={feedScope === 'all' ? styles.feedScopeActive : ''}
              onClick={() => void changeFeedScope('all')}
            >
              全部动态
            </button>
            <button
              type="button"
              className={feedScope === 'friends' ? styles.feedScopeActive : ''}
              onClick={() => void changeFeedScope('friends')}
            >
              <FiUsers /> 好友动态
            </button>
          </div>
          <div className={styles.feedList}>
            {visiblePosts.length ? (
              visiblePosts.map((post) => (
                <article className={styles.momentPost} key={post.id}>
                  <header>
                    <MomentAvatar post={post} onUser={onUser} />
                    <button
                      type="button"
                      className={styles.momentIdentityButton}
                      onClick={() => onUser(Number(post.userId))}
                    >
                      <strong>
                        {post.username}
                      ...
                      <aside>
                        {Number(post.userId) === Number(userId) ? (
                          ...
                          <>
                            ...
                            <button
                              type="button"
                              disabled={reportingId === post.id || reportedIds.includes(post.id)}
                  ...
                  <button
                    type="button"
                    className={styles.momentBodyButton}
                  ...
                  {post.images.length ? (
                    <div
                      className={`${styles.momentPhotoGrid} ${post.images.length === 1 ? styles.momentPhotoSingle : ''}`}
                    >
                      {post.images.slice(0, 6).map((image) => (
                        <img key={`${post.id}-${image.url}`} src={image.url} alt="动态图片" />
                      ))}
                    </div>
                  ) : (
                    <div
                      className={styles.momentVisualFallback}
                      style={{ '--moment-color': post.color || '#4f7cff' } as React.CSSProperties}
                    >
                      <FiImage />
                  ...
                  {(comments[post.id] || []).length ? (
                    <div className={styles.commentPreview}>
                      ...
                      {commentPages[post.id]?.page < commentPages[post.id]?.lastPage ? (
                        <button
                          type="button"
                          onClick={() => void loadComments(post.id, commentPages[post.id].page + 1)}
                        >
                          加载更多评论
                        </button>
                      ) : null}
                  ...
                  <footer>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setCommentText('');
                        if (opening && !comments[post.id]) void loadComments(post.id);
                      }}
                    >
                      <FiMessageCircle /> {post.comments}
                    </button>
                    <button type="button" onClick={() => void sharePost(post)}>
                      <FiSend /> 分享
                    </button>
                  </footer>
                </article>
            ...
            <div className={styles.paginationFooter}>
              {currentPage < lastPage ? (
                <button type="button" disabled={loadingMore} onClick={() => void loadMore()}>
                  {loadingMore ? '正在加载…' : '查看更多运动生活'}
                </button>
              ) : visiblePosts.length ? (
                <span>已经看到今天最新动态</span>
              ) : null}
            </div>
          </div>
        </>
