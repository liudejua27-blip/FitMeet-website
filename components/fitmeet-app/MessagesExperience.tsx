...
export function MessagesExperience({ invitations, conversations, incomingConnections, outgoingConnections, agentEvents, ownerSocialApplications, ownerTaskApplications, currentUserId, unreadCount, onConversation, onInvitation, onIntentApplication, onAcknowledge, onMeet, onRelationship, onRefresh }: {
  ...
  const searchItems = useMemo<SearchItem[]>(() => [
    ...
    ...agentEvents.map((item) => ({ id: `agent-event-${item.id}`, category: "system" as const, title: item.title || "FitMeet 通知", subtitle: item.body || item.type || "账号状态已更新", unread: 1, action: () => onAcknowledge(item.id) })),
  ], [agentEvents, conversations, incomingConnections, onAcknowledge, onConversation, onMeet, onRelationship, outgoingConnections, pendingReceived, pendingSent, pendingSocialApplications, pendingTaskApplications]);
  ...
  return <div className={styles.standardScreen}>
    <header className={styles.messageHeader}><div><h1>消息</h1><p>{unreadCount ? `${unreadCount} 条未读` : "私信、互动和通知"}</p></div><button type="button" aria-label="刷新消息" onClick={() => void refresh()} className={refreshing ? styles.spinIcon : ""}><FiRefreshCw /></button></header>
    <button type="button" className={styles.searchButton} onClick={() => setSearchOpen(true)}><FiSearch /> 搜索私信、约练或系统通知</button>
    <section className={styles.messageOverview}><span><strong>{unreadCount + agentEvents.length}</strong><small>未读</small></span><span><strong>{conversations.length}</strong><small>私信</small></span><span><strong>{interactionCount}</strong><small>互动</small></span><span><strong>{agentEvents.length}</strong><small>系统</small></span></section>
    ...
    {agentEvents.length ? <><h2 className={styles.listTitle}>系统通知</h2>{agentEvents.slice(0, 5).map((item) => <button type="button" className={styles.messageRow} key={item.id} onClick={() => onAcknowledge(item.id)}><span className={styles.messageInteractionIcon}><FiBell /></span><span><strong>{item.title || "FitMeet 通知"}</strong><small>{item.body || item.type || "账号状态已更新"}</small></span><time className={styles.unreadBadge}>1</time><FiChevronRight /></button>)}</> : null}
