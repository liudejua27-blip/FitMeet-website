...
const sections = [
  ...
  },
  {
    ...
    body: [
      "服务名称：DeepSeek 模型 API。角色：为 FitMeet Agent 提供语言理解与生成计算。使用场景：用户主动发起 Agent 对话、需求草稿整理、候选理由解释与安全控制。客户端不保存或公开模型密钥。",
      "可能处理的信息：用户主动输入的文字、语音转写文本、主动附加的图片说明、完成当前请求所需的会话上下文、经确认的资料与需求字段，以及请求时间、模型结果和必要安全元数据。不会为无关目的发送登录密码、精确住址、证件或整套私聊记录。",
      <>发布核验：模型服务的签约主体、处理地域、日志保存和数据使用条款必须以实际 API 采购合同与控制台订单核验并留档；本清单不自行推定法定主体。核验未完成时，不得向该能力开放包含个人信息的公众生产流量。服务公开规则可查看<a href="https://cdn.deepseek.com/policies/zh-CN/deepseek-privacy-policy.html" target="_blank" rel="noreferrer">DeepSeek 隐私政策</a>。</>,
    ],
  },
  {
    index: "05",
  ...
  },
  {
    ...
    body: [
      ...
      <>本清单版本为 1.0，更新及生效日期为 2026-07-29。历史政策版本可在<Link href="/legal/versions">法律文件版本记录</Link>中查阅。</>,
    ],
  },
];
